...
final class RealCoordinateConverterTest: XCTestCase {
  private let lngLatOrigin = CLLocationCoordinate2D(latitude: 59.35293, longitude: 17.95590)
  ...
  func test_that_conversion_from_meter_to_latlng_and_back_again_100_000_times_gives_us_accetable_and_expected_result() {
    ...
    let originalLngLatCoordinates = originalMeterCoordinates.map({ converter.metersToLngLat(point: $0) })
    XCTAssertEqual(originalLngLatCoordinates, expectedLngLatCoordinates)
    var coordinates: Any = originalLngLatCoordinates.copy()

    for _ in 1..<convertCount - 1 {
      coordinates = (coordinates as! [CLLocationCoordinate2D]).map({ converter.lngLatToMeters(coordinate: $0) })
      coordinates = (coordinates as! [CGPoint]).map({ converter.metersToLngLat(point: $0) })
    }

    coordinates = (coordinates as! [CLLocationCoordinate2D]).map({ converter.lngLatToMeters(coordinate: $0) })
    XCTAssertEqual((coordinates as! [CGPoint]), m100_000ExpectedMeterCoordinates)
    coordinates = (coordinates as! [CGPoint]).map({ converter.metersToLngLat(point: $0) })
    XCTAssertEqual((coordinates as! [CLLocationCoordinate2D]), m100_000ExpectedLngLatCoordinates)
  }
  ...
  // Original lngLat coordinates:
  private let expectedLngLatCoordinates: [CLLocationCoordinate2D] = [
  ...
  // 100 000 lngLat coordinates:
  private let m100_000ExpectedLngLatCoordinates: [CLLocationCoordinate2D] = [
    .init(latitude: 59.35293000000001, longitude: 17.9559),
    .init(latitude: 59.35356698317859, longitude: 17.954115313635846),
    .init(latitude: 59.35439497096183, longitude: 17.95525268955188),
    .init(latitude: 59.35375806207805, longitude: 17.95703739810693),
